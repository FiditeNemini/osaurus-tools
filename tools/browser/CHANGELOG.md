...
- **Deterministic failures are no longer flagged retryable.** The normalization boundary marked every rewritten failure `retryable: true`, including `invalid_args` (and unknown-tool lookups, now classified `not_found`). Retrying an identical call with the same bad arguments can never succeed, so those kinds now carry `retryable: false`; transient `execution_error` failures remain retryable.
